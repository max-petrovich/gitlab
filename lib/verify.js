...
const VALIDATORS = {
  assets: isArrayOf(
    asset => isStringOrStringArray(asset)
        ||
        (
            isPlainObject(asset)
            && isStringOrStringArray(asset.path)
            &&
            (
                !asset.registry
                ||
                (
                    isNonEmptyString(asset.registry) ||
                    (
                      isPlainObject(asset.registry)
                      && isNonEmptyString(asset.registry.package_name)
                    )
                )
            )
        )
