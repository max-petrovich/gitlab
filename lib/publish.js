  ...
  if (assets && assets.length > 0) {
    ...
    await Promise.all(
      globbedAssets.map(async asset => {
        const {path, label, type, filepath, package_registry} = isPlainObject(asset) ? asset : {path: asset};
        ...
        debug('package registry: %o', package_registry);

        // Uploaded assets to the project
        const form = new FormData();
        form.append('file', createReadStream(file));

        const uploadEndpoint = urlJoin(gitlabApiUrl, `/projects/${encodedRepoId}/${package_registry ? `packages/generic/${package_registry}/${encodedGitTag}/${label}` : 'uploads'}`);

        let response;
        const method = package_registry ? 'put': 'post';

        debug(method.toUpperCase() + '-ing the file %s to %s', file, uploadEndpoint);

        try {
          response = await got(uploadEndpoint, {...apiOptions, body: form, method}).json();
        } catch (error) {
          logger.error('An error occurred while uploading %s to the GitLab project uploads API:\n%O', file, error);
          throw error;
        }

        const {url, alt} = package_registry ? {url: uploadEndpoint, alt: label} : response;
  ...
  const json = {
    ...
    assets: {
      links: assetsList.map(({label, alt, url, type, filepath}) => {
        return {
          name: label || alt,
          url: url.indexOf(gitlabApiUrl) === 0 ? url: urlJoin(gitlabUrl, repoId, url),
          link_type: type,
          filepath,
        };
      }),
