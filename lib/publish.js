  ...
  if (assets && assets.length > 0) {
    ...
    await Promise.all(
      globbedAssets.map(async asset => {
        ...
        const registry = asset.registry ? isPlainObject(asset.registry) ? asset.registry : {package_name: "release"} : undefined;
        ...
        debug('package registry', registry);

        if (registry) {
            // Uploaded assets to the project via package registry
            debug(tagFormat);
            const packageVersion = encodeURIComponent(tagFormat === `v\${version}` ? gitTag.substring(1) : gitTag);
            debug(packageVersion);
            const uploadEndpoint = urlJoin(gitlabApiUrl, `/projects/${encodedRepoId}/packages/generic/${registry.package_name}/${encodedGitTag}/${label}`);

            debug('PUT-ing the file %s to %s', file, uploadEndpoint);

            try {
                await got.put(uploadEndpoint, {...apiOptions,  body: createReadStream(file)}).json();
            } catch (error) {
                logger.error('An error occurred while uploading %s to the GitLab package registry:\n%O', file, error);
                throw error;
            }

            assetsList.push({label, alt: label, url: uploadEndpoint, type, filepath});

            logger.log('Uploaded file to package registry: %s', uploadEndpoint);
        } else {
            // Uploaded assets to the project (directly)
            ...
            assetsList.push({label, alt, url: urlJoin(gitlabUrl, repoId, url), type, filepath});

            logger.log('Directly uploaded file to project: %s', url);
        }
      })
    );
  }
